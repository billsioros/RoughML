...
from roughgan.content.loss import VectorSpaceContentLoss
from roughgan.plot import animate_epochs, as_3d_surface, as_grayscale_image, plot_against
from roughgan.shared.configuration import Configuration
from roughgan.shared.context_managers import ExceptionLoggingHandler
from roughgan.training.manager import TrainingManager
...
class TrainingFlow(Configuration):
    ...
    def __call__(self, get_generator, get_discriminator):
        for path, dataset in self.data.loader():
            ...
            with ExceptionLoggingHandler(
                ...
                dataset_output_dir = self.process_dataset(generator, discriminator, path, dataset)
    ...
    def process_dataset(self, generator, discriminator, path, dataset):
        ...
        if self.plot.against.save_path_fmt is not None:
            save_path_gen_vs_dis_loss = self.plot.against.save_path_fmt % ("gen_vs_dis_loss",)
            save_path_bce_vs_con_loss = self.plot.against.save_path_fmt % ("bce_vs_con_loss",)
            save_path_dis_output = self.plot.against.save_path_fmt % ("discriminator_output",)
            save_path_vector_loss = self.plot.against.save_path_fmt % ("content_vs_vector_loss",)
        ...
        if self.plot.save_directory is not None:
            ...
            if self.plot.surface.save_path_fmt is not None:
                (self.plot.save_directory / self.plot.surface.save_path_fmt).parent.mkdir(
                    parents=True,
                    exist_ok=True,
                )
                ...
                for i in range(self.plot.surface.limit):
                    ...
                    as_3d_surface(
                        true_surface,
                        self.plot.save_directory / (self.plot.surface.save_path_fmt % ("true", i)),
                    )
                    as_3d_surface(
                        fake_surface,
                        self.plot.save_directory / (self.plot.surface.save_path_fmt % ("fake", i)),
                    )
